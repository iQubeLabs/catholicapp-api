...
define('NG_CURRENCY_CODE', '566'); //566 => Naira
define('US_CURRENCY_CODE', '840'); //566 => Naira
define('BR_CURRENCY_CODE', '826'); //566 => Naira
define('ORDER_ID_PRESTRING', 'CAPP'); //566 => Naira
define('NG_AMOUNT_YEARLY', 2000); //566 => Naira
define('NG_AMOUNT_MONTHLY', 200); //566 => Naira
define('USD_AMOUNT_YEARLY', 1.5); //840 => Dollars
define('USD_AMOUNT_MONTHLY', 15); //840 => Dollars
define('PND_AMOUNT_YEARLY', 10); //826 => Pound Sterling
define('PND_AMOUNT_MONTHLY', 1); //826 => Pound Sterling
...
function getPaymentParams($subType, $currCode) {

    if($currCode == 'NG') {

        $currency = NG_CURRENCY_CODE;
        if($subType == 'MTH') {
            $amount = NG_AMOUNT_MONTHLY;
            $prod_prefix = 'Monthly Payment for';
        } else if($subType == 'YR') {
            $amount = NG_AMOUNT_YEARLY;
            $prod_prefix = 'Yearly Payment for';
        }
    } else if($currCode == 'USD') {

        $currency = US_CURRENCY_CODE;
        if($subType == 'MTH') {
            $amount = USD_AMOUNT_MONTHLY;
            $prod_prefix = 'Monthly Payment for';
        } else if($subType == 'YR') {
            $amount = USD_AMOUNT_YEARLY;
            $prod_prefix = 'Monthly Payment for';

        }
    } else if($currCode == 'PND') {

        $currency = BR_CURRENCY_CODE;
        if($subType == 'MTH') {
            $amount = PND_AMOUNT_MONTHLY;
            $prod_prefix = 'Monthly Payment for';
        } else if($subType == 'YR') {
            $amount = PND_AMOUNT_YEARLY;
            $prod_prefix = 'Monthly Payment for';
    ...
    return array(
        ...
        'currCode' => $currency,
        'amt' => $amount,
        'prod' => "{$prod_prefix} Catholic Android App.",
        'submit' => 'Pay',
        'orderId' => '',
        'email' => ''
    );
        ...
        if(isset($phone) && isset($password)) {
            ...
            if(isset($user)) {
                ...
                if(isset($payment)) {
                    ...
                    $subscription['expiry_date'] = $payment->sub_expiry;
                } else {
                ...
                $userArray['subscription'] = $subscription;
...
$app->get('/order/new', function() use ($app) {
    ...
    try {
        ...
        $currCode = $app->request->get('currCode');
        ...
        $params = getPaymentParams($subType, $currCode);
...
$app->get('/transaction/handler', function() use ($app) {
    ...
    if($trans->status_code == '00') {
        ...
        $order = PaymentOrder::where('order_id', '=', $trans->order_id)->first();
        $payment->user_id = $order->user_id;
        if($order->sub_type == 'MTH') {
            $expirydate = strtotime(MONTHLY_EXPIRY_PERIOD);
        } else if($order->sub_type == 'YR') {
            $expirydate = strtotime(YEARLY_EXPIRY_PERIOD);
        }
        $str_expirydate = date('Y-m-d', $expirydate);
        $payment->sub_expiry = $str_expirydate;
        $payment->save();
